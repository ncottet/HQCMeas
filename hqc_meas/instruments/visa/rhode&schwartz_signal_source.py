...
:Contains:
    RhodeSchwartzSignalGenerator : tested for Agilent
...
class RhodeSchwartzSignalGenerator(VisaInstrument):
    """
    Generic driver for Rhode&Schwartz SignalGenerator, using the VISA library.
    ...
    Notes
    -----
    This driver has been written for the Rhode&Scwartz SGMA100A  but might work
    for other models using the same SCPI commands.

    """
    def __init__(self, connection_info, caching_allowed=True,
                 caching_permissions={}, auto_open=True):

        super(RhodeSchwartzSignalGenerator, self).__init__(connection_info,
                                                        caching_allowed,
                                                        caching_permissions,
                                                        auto_open)
